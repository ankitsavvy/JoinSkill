  ...
  return (
    <article className="person-card">
      ...
      <div className="person-actions">
        <Button variant="secondary" icon={MessageCircle} type="button" onClick={() => onMessage(person)}>
          Message
        </Button>
        <Button variant="primary" icon={Video} type="button" onClick={() => onVideo(person)}>
          Video
        </Button>
      </div>
    </article>
...
function DiscoverView({ token, onMessage, onVideo }) {
  ...
  async function search() {
    setLoading(true);
    try {
      const data = await apiRequest(`/api/users?skill=${encodeURIComponent(query)}&intent=${intent}`, { token });
    ...
    <div className="dashboard-stack">
      <section className="panel search-panel">
        ...
        <div className="search-row">
          ...
          />
          <Button icon={loading ? Loader2 : Search} onClick={search} className={loading ? "is-loading" : ""}>
            Search
          </Button>
        </div>
        <div className="suggestion-row">
    ...
    <div className="modal-backdrop">
      <section className="video-modal">
        <header className="video-header">
          ...
          <IconButton icon={X} label="Close session" onClick={onClose} />
        </header>
